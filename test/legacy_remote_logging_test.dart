...
import 'package:remote_logging/src/remote/tags/impl/io_tags.dart';
...
void main() {
  ...
  test('root severe', () async {
    ...
    expect(lastHeaders!['X-LOGGLY-TAG']![0], 'SEVERE,${defaultTags.join(',')}');
  });
  ...
  test('tags', () async {
    ...
    await Future.delayed(Duration.zero);
    expect(
      lastHeaders!['X-LOGGLY-TAG']![0],
      'SEVERE,${[...defaultTags, ...tags].join(',')}',
    );
  });
  ...
  test('loggers', () async {
    ...
    for (final logger in loggers) {
      ...
      expect(
        lastHeaders!['X-LOGGLY-TAG']![0],
        'SEVERE,${[...defaultTags, logger].join(',')}',
      );
    }
  });
}
